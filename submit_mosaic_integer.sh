...
# This file calls mosaic_integer.sh which has compression options for integer, without this option gdal_translate throws an error, and should be run with mosaic_filenames1_integer.csv
...
for line in $csv_data ; do
    ...
    if [ ! -f "$desFol/$name.tif" ] || ! gdalinfo "$desFol/$name.tif" &>/dev/null; then
        srun --ntasks=1 --nodes=1 --exclusive ./mosaic_integer.sh "$line" "$desFol" &> "${logFol}/mosaic.${name}.out" &
    fi
done
